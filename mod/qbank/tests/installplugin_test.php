...
class installplugin_test extends \advanced_testcase {
    ...
    public function test_populated_only(): void {
        ...
        $retrievedcat = data_migration_helper::get_categories_populated();
    ...
    public function test_course_creation(): void {
        ...
        $courseexists = data_migration_helper::get_course($course1->shortname, $category1->id);
        $this->assertEquals($course1->shortname, $courseexists->shortname);

        // Test that course does not exist before being created.
        $course2 = data_migration_helper::get_course($courseshortname2, $category2->id);
        $this->assertNull($course2);

        // Test that course exists after being created with create_category_course function.
        if (!$course2) {
            $course2 = data_migration_helper::create_category_course($courseshortname2, $category2->id);
        }
    ...
    function test_qbank_instancecreated(): void {
        ...
        $qbank = data_migration_helper::create_qbank_instance($courseshortname, $course);
    ...
    function test_question_categories_migration(): void {
        ...
        $qbanksystemcontext = data_migration_helper::create_qbank_instance('Question bank System context', get_course(SITEID));
        $categorycourse = data_migration_helper::create_category_course('Category course', $category->id);
        $qbankcategorycontext = data_migration_helper::create_qbank_instance('Question bank Category context', $categorycourse);
        $qbankcoursecontext = data_migration_helper::create_qbank_instance('Question bank System context', $course);
        $qbankquizcontext = data_migration_helper::create_qbank_instance('Question bank Quiz context', $course);
        ...
        data_migration_helper::migrate_question_categories($qbanksystemcontext, $systemcontext);
        ...
        data_migration_helper::migrate_question_categories($qbankcategorycontext, $categorycontext);
        ...
        data_migration_helper::migrate_question_categories($qbankcoursecontext, $coursecontext);
        ...
        data_migration_helper::migrate_question_categories($qbankquizcontext, $quizcontext);
