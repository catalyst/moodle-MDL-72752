...
use mod_qbank\data_migration_helper;
...
function xmldb_qbank_install() {
    ...
    $questioncategories = \mod_qbank\data_migration_helper::get_categories_populated();
    foreach ($questioncategories as $qcategory) {
        ...
        switch ($contextlevel) {
            ...
            case CONTEXT_COURSECAT:
                ...
                $shortdescription = data_migration_helper::get_category_name($qcategory->instanceid);
                ...
                $course = data_migration_helper::get_course($shortname, $qcategory->instanceid);
                if (!$course) {
                    $course = data_migration_helper::create_category_course($shortname, $qcategory->instanceid);
                }
                break;
            case CONTEXT_COURSE:
                ...
                break;
            case CONTEXT_MODULE:
                $cm = data_migration_helper::get_coursemodule($qcategory->instanceid);
        ...
        $qbank = data_migration_helper::create_qbank_instance($qbankname, $course);

        // Migrate the question_categories data to new mod_qbank.
        if ($qbank) {
            data_migration_helper::migrate_question_categories($qbank, $qcategory->contextid);
        }
    }
