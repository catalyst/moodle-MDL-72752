...
class view {
    ...
    protected function build_query(): void {
        ...
        $latestversion = 'qv.version = (SELECT MAX(v.version)
                                         ...
                                         WHERE be.id = qbe.id)';
        $nothidden = '((' . "qv.status <> '" . question_version_status::QUESTION_STATUS_HIDDEN . "'" .'))';
        $this->sqlparams = [];
        $conditions = [];
        foreach ($this->searchconditions as $searchcondition) {
            if ($searchcondition->where()) {
                if ($searchcondition->get_condition_key() === 'hidden') {
                    $nothidden = '((' . $searchcondition->where() .'))';
                } else {
        ...
        $majorconditions = ['q.parent = 0', $latestversion, $nothidden];
